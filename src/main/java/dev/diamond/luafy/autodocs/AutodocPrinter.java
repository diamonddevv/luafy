...
public class AutodocPrinter {
    ...
    public static final Autodoc<?, ?>[] AUTODOCS = {
            new JsonAutodoc("luafy_autodocs.json")
    };
    ...
    public static void printDocs(Predicate<Identifier> idPredicate) {
        ...
        for (Autodoc<?, ?> autodoc : AUTODOCS) {
            // do file write
            try {
                writeDoc(autodoc, idPredicate);

                Luafy.LOGGER.info("Written " + autodoc.getFilename() + " autodoc.");
            } catch (IOException e) {
    ...
    private static <F, A> void writeDoc(Autodoc<F, A> autodoc, Predicate<Identifier> idPredicate) throws IOException {
        ...
        for (var api : Luafy.Registries.API_REGISTRY) {
            if (!idPredicate.test(Luafy.Registries.API_REGISTRY.getId(api))) continue;
        ...
        for (var o : Luafy.Registries.SCRIPT_OBJECTS_REGISTRY) {
            if (!idPredicate.test(Luafy.Registries.SCRIPT_OBJECTS_REGISTRY.getId(o))) continue;
        ...
        for (var o : Luafy.Registries.CALLBACK_REGISTRY) {
            if (!idPredicate.test(Luafy.Registries.CALLBACK_REGISTRY.getId(o))) continue;
            autodoc.addEvent(o, events);
        }

        for (var o : Luafy.Registries.SCRIPT_LANG_REGISTRY) {
            if (!idPredicate.test(Luafy.Registries.SCRIPT_LANG_REGISTRY.getId(o))) continue;
            autodoc.addLang(o, langs);
        }

        autodoc.addList(obj, apis, "api");
        autodoc.addList(obj, objs, "script_objects");
        autodoc.addList(obj, events, "callback_events");
        autodoc.addList(obj, langs, "script_languages");
