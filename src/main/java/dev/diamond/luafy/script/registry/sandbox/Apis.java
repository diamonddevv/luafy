...
public class Apis {
    ...
    public static void registerAll() {
        ...
        Registry.register(Luafy.API_REGISTRY, Luafy.id("lua_package"), LUA_PACKAGE);
        Registry.register(Luafy.API_REGISTRY, Luafy.id("lua_bit32"), LUA_BIT32);
        Registry.register(Luafy.API_REGISTRY, Luafy.id("lua_table"), LUA_TABLE);
        Registry.register(Luafy.API_REGISTRY, Luafy.id("lua_string"), LUA_STRING);
        Registry.register(Luafy.API_REGISTRY, Luafy.id("lua_coroutine"), LUA_COROUTINE);
        Registry.register(Luafy.API_REGISTRY, Luafy.id("lua_math"), LUA_MATH);
        Registry.register(Luafy.API_REGISTRY, Luafy.id("lua_io"), LUA_IO);
        Registry.register(Luafy.API_REGISTRY, Luafy.id("lua_os"), LUA_OS);
        Registry.register(Luafy.API_REGISTRY, Luafy.id("lua_luajava"), LUA_LUAJAVA);
        Registry.register(Luafy.API_REGISTRY, Luafy.id("lua_loadstate"), LUA_LOADSTATE);
        Registry.register(Luafy.API_REGISTRY, Luafy.id("lua_c"), LUA_C);
        Registry.register(Luafy.API_REGISTRY, Luafy.id("luafy"), LUAFY);
        Registry.register(Luafy.API_REGISTRY, Luafy.id("command"), COMMAND);
        Registry.register(Luafy.API_REGISTRY, Luafy.id("server"), SERVER);
        Registry.register(Luafy.API_REGISTRY, Luafy.id("context"), CONTEXT);
        Registry.register(Luafy.API_REGISTRY, Luafy.id("storage"), STORAGE);
    ...
    private static List<SandboxableApi<?>> parseStrategy(Strategy strategy) {
        ...
        if (strategy.blacklist) {
            m = Luafy.API_REGISTRY.getIds().stream().map(Identifier::toString).toList();
        ...
        List<SandboxableApi<?>> apis = new ArrayList<>(Luafy.API_REGISTRY.stream().filter(SandboxableApi::alwaysLoads).toList());

        for (Identifier id : m.stream().map(Identifier::new).toList()) {
            SandboxableApi<?> api = Luafy.API_REGISTRY.get(id);
            if (api != null) {
                apis.add(api);
            }
        }
